...
function calculate(input) {
  ...
  const mult= "*";
  const sum = "+";
  const deduct = "-"; //subtrair
  const division = "/"
 
  while(copyInput.includes(deduct)){
      
    let limits = {
       begin: 0,
       end: 0
    }
    if(copyInput.includes(mult)){
      let resultmult = resultOperator(copyInput, mult, multiply, limits);
      copyInput.splice(limits.begin, limits.end+1, resultmult);
      //total += resultmult;

    }else if(copyInput.includes(division)){
...
function resultOperator(copyInput, compareOperador, mathOperationFunction, limits){
  ...
  console.log(`getNumberBefore ${JSON.stringify(limits)}`);
  let numberAfter = getNumberAfter(copyInput, indexOperator, limits);
  console.log(`getNumberAfter ${JSON.stringify(limits)}`);
